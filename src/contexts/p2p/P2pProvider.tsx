...
function P2pProvider({ children }: any) {
        ...
        if (peerId.toString() === '[object Object]') {
            ...
            getOrCreatePeerId().then((pi) => {
                ...
                .then(async (peerId) => {
                    ...
                    //const peerLocal =  await PeerID.createFromPrivKey(peerId)
                    return init(peerId)
                })
    ...
    const init = async (peer: any) => {
        console.log("🚀 ~ file: P2pProvider.tsx ~ line 87 ~ init ~ peer", peer)
        ...
        try {
            ...
            node.connectionManager.addEventListener('peer:connect', (evt) => {
                ...
                const tmpPeers = foundPeers;
                tmpPeers.add(connection.remotePeer.toString())
                setFoundPeers(tmpPeers)
                dispatch({
                    type: ActionKind.init,
                    payload: { libp2p: node, peerId: node.peerId.toString(), connectedPeers: foundPeers }
                });
            })
            ...
            node.connectionManager.addEventListener('peer:disconnect', (evt) => {
                ...
                const tmpPeers = foundPeers;
                tmpPeers.delete(connection.remotePeer.toString())
                setFoundPeers(tmpPeers)
                dispatch({
                    type: ActionKind.init,
                    payload: { libp2p: node, peerId: node.peerId.toString(), connectedPeers: foundPeers }
            ...
            const outputMessage = (msg: string) => {
                dispatch({
                    type: ActionKind.init,
                    payload: { libp2p: node, peerId: node.peerId.toString(), connectedPeers: foundPeers, messages: []}
                });
            }
            // Handle messages for the protocol
            await node.handle('/chat/1.0.0', async ({ stream }) => {
                streamToConsole(stream, outputMessage)
            ...
            dispatch({
                type: ActionKind.init,
                payload: { libp2p: node, peerId: node.peerId.toString(), connectedPeers: [] }
            });
        } catch (err) {
